 ...
 */
async function detectSlashingEvent(validatorAddr) {
    try {
 ...
 */
async function reportSlashingEvent(policyId, txHash) {
    const evidenceHash = keccak256(toUtf8Bytes(txHash));
...
async function pollOnce() {
    ...
    await Promise.allSettled(ids.map(async (policyId) => {
        ...
        if (Number(policy.status) !== STATUS_ACTIVE)
            return;
        if (Number(rawCovType) !== COVERAGE_TYPE_SLASHING)
            return;
