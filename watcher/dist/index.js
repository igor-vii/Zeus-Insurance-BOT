...
const signer = new ethers.Wallet(WATCHER_PRIVATE_KEY.startsWith("0x") ? WATCHER_PRIVATE_KEY : `0x${WATCHER_PRIVATE_KEY}`, provider);
...
async function probeEndpoint(url, timeoutMs = 5000) {
...
function httpStatusToObsStatus(httpStatus) {
    if (httpStatus === 0)
        return 1; // TIMEOUT
    if (httpStatus >= 500)
        return 2; // ERROR_500
 ...
 */
async function buildObservation(buyer, seller, obsStatus, metadataExtra = "") {
    const timestamp = BigInt(Math.floor(Date.now() / 1000));
    nonce++;
    // requestId = keccak256(abi.encodePacked(buyer, seller, timestamp))
    const requestId = ethers.keccak256(ethers.solidityPacked(["address", "address", "uint256"], [buyer, seller, timestamp]));
    ...
    const msgHash = ethers.keccak256(ethers.solidityPacked(["bytes32", "uint256", "uint8", "bytes32", "uint256"], [requestId, timestamp, obsStatus, metadataHash, nonce]));
...
async function submitObservation(policyId, obs) {
...
async function pollOnce() {
    ...
    for (let id = start; id < nextId; id++) {
        ...
        if (policy.status !== STATUS_ACTIVE)
            continue;
        if (Number(policy.retryDeadline) > now)
            continue;
        ...
        try {
            ...
            log(`Policy ${id}: submission failed ❌  ${err.message}`);
        }
...
async function main() {
    ...
    await pollOnce().catch(e => log(`Poll error: ${e.message}`));
    setInterval(() => pollOnce().catch(e => log(`Poll error: ${e.message}`)), POLL_INTERVAL_MS);
}
main().catch(e => {
