...
export const apiWatcher = {
  ...
  async check(policy: Policy): Promise<WatcherVote> {
    try {
      ...
      if (!resp.ok) return { watcher: 'api', vote: 'abstain', reason: `HTTP ${resp.status}` };
      
      const data = await resp.json();
      if (data.deliveryFailed === true) return { watcher: 'api', vote: 'yes', reason: 'API: delivery failed' };
      if (data.delivered === true) return { watcher: 'api', vote: 'no', reason: 'API: delivered' };
